...
export const Expenses: React.FC = () => {
  ...
  const [form, setForm] = useState<Partial<ExpenseTransaction>>({
    amount: 0,
  ...
  const fetchTransactions = async () => {
    ...
    if (data) {
      setTransactions(data.map(t => ({
        ...t,
        category_name: t.expense_categories?.name || 'Uncategorized'
      })));
    }
  };
  ...
  const handleSave = async () => {
    ...
    let error;
    if (editingTxn) {
      const { error: err } = await supabase.from('expenses').update(data).eq('id', editingTxn.id);
      error = err;
    } else {
    ...
    if (error) {
      ...
      alert('Failed to save expense record. ' + error.message);
    } else {
  ...
  const handleSaveCategory = async () => {
    ...
    if (editingCategory) {
      await supabase.from('expense_categories').update(categoryForm).eq('id', editingCategory.id);
    } else {
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500">Track all institutional expenses and manage categories.</p>
        </div>
        <div className="flex items-center gap-3">
          {activeTab === 'transactions' ? (
            <button 
              ...
              className="flex items-center gap-2 px-4 py-2 bg-rose-600 text-white rounded-xl text-sm font-bold hover:bg-rose-700 transition-colors shadow-lg shadow-rose-200"
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex items-center gap-1 text-xs font-bold px-2 py-1 rounded-full text-rose-600 bg-rose-50">
                  <ArrowDownRight className="w-3 h-3" />
                  +8%
                </div>
              </div>
              <h3 className="text-slate-500 text-sm font-medium mb-1">Total Expenses (MTD)</h3>
            ...
            <div className="flex flex-col sm:flex-row items-center gap-4 flex-1">
              <div className="relative w-full sm:max-w-md">
                ...
                <input 
                  ...
                  className="w-full pl-10 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-rose-500/20 outline-none"
                />
              </div>
              <select 
                value={categoryFilter}
                onChange={(e) => setCategoryFilter(e.target.value)}
                className="px-4 py-2 bg-slate-50 border border-slate-200 text-slate-600 rounded-lg text-sm font-medium outline-none focus:ring-2 focus:ring-rose-500/20"
              >
              ...
              <tbody className="divide-y divide-slate-100">
                {filteredTransactions.map((t) => (
                  <tr key={t.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-sm font-bold text-rose-600 text-right">{formatCurrency(t.amount)}</td>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
            ...
            >
              <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-rose-50/50">
                <h3 className="text-lg font-bold text-rose-900">{editingTxn ? 'Edit Expense' : 'Add New Expense'}</h3>
                <button onClick={() => setIsModalOpen(false)} className="p-2 hover:bg-rose-100 rounded-lg transition-colors">
              ...
              <div className="p-6 space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                    ...
                    <input 
                      ...
                      className="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-rose-500/20 outline-none"
                    />
                  </div>
                  <div className="space-y-1">
                ...
                <div className="space-y-1">
                  ...
                  <textarea 
                    ...
                    rows={3}
                    className="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-rose-500/20 outline-none resize-none"
              ...
              <div className="p-6 bg-slate-50 border-t border-slate-100 flex items-center justify-end gap-3">
                ...
                <button 
                  onClick={handleSave}
                  className="flex items-center gap-2 px-6 py-2 bg-rose-600 text-white rounded-xl text-sm font-bold hover:bg-rose-700 transition-colors shadow-lg shadow-rose-200"
                >
                  <Save className="w-4 h-4" />
                  {editingTxn ? 'Update Expense' : 'Save Expense'}
                </button>
              </div>
            </motion.div>
          </div>
              ...
              <div className="p-6 space-y-4">
                <div className="space-y-1">
                  ...
                  <input 
                    ...
                    placeholder="e.g. Salary, Maintenance, etc."
                  />
                </div>
                <div className="space-y-1">
                  <label className="text-xs font-bold text-slate-500 uppercase">Description</label>
